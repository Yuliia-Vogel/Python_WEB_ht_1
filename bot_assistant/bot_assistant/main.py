...
class UserInterface(ABC): # створюємо абстрактний клас для легкої модифікації виведеня інформації в консоль
    @abstractmethod # цей декоратор тут для того, щоб не можна було створити екземпляр цього класу, бо він абстрактний
    def show_to_user(self): # пишемо функцію, яку потім потрібно буде обов"язково перевизначити 
        pass

class ContactsUserInterface(UserInterface): # клас для відображення інформації по контактах
    def __init__(self, contact): # в ініті визначаємо поле contact
        self.contact = contact 

    def show_to_user(self): # власне відображення інформації про контакт
        print("*-"*25)
...
class NoteUserInterface(UserInterface): # клас для відображення інформації по нотатках
    def __init__(self, note): # в ініті визначаємо поле note
        self.note = note

    def show_to_user(self): # відображення інформації, що міститься в нотатці
        print("*-"*25)
...
class HelpUserInterface(UserInterface): # клас для відображення інформації за командою help
    def __init__(self, help_dict): # в ініті визначаємо поле help_dict
        self.help_dict = help_dict

    def show_to_user(self): # відображення інформації по команді help
        print("_"*50)
...
class ContactBot:
    ...
    def search_note(self, search_query):
        ...
        if found_notes:
            for note in found_notes: 
                return note # тепер нотатки повертаються саме як об"єкт "нотатка"
            # return "\n".join(str(note) for note in found_notes) # раніше так показулись знайдені нотатки
        else:
    ...
    def help(self):
        ...
        # # Відображаємо відформатований список команд
    ...
    def search_contacts(self, name):
        ...
        if found_records:
            for record in found_records:
                return record # тепер пошук повертає рекорд, щоб функція show_to_user з класу ContactsUserInterface могла красиво відобразити контакт за заданим шаблоном
            # return "\n".join(str(record) for record in found_records) # раніше повертали таку стрінгу в якості результату пошуку контакту
        else:
